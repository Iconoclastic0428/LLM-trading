...
def render_year_chart(frame: pd.DataFrame, annual: pd.Series, output: Path) -> None:
    ...
    ax.plot(frame["date"], frame["strategy_return_index"], color=COLORS["strategy"], linewidth=2.5, label="Momentum Select")
    ...
    fig.suptitle(
        f"Momentum Select vs QQQ and SPY — {int(annual['year'])}",
        fontsize=16,
        fontweight="bold",
        y=0.98,
    )
    ...
    fig.savefig(output, metadata={"Software": "momentum_select_strategy/scripts/render_reports.py"})
    plt.close(fig)


def render_cumulative_chart(daily: pd.DataFrame, output: Path) -> None:
    fig, ax = plt.subplots(figsize=(15, 8), dpi=120)
    ax.plot(daily["date"], daily["strategy_equity"], color=COLORS["strategy"], linewidth=2.4, label="Momentum Select")
    ...
    ax.set_xlabel("Session")
    ax.set_title("Momentum Select — cumulative real-500 replay")
...
def render_annual_returns(annual: pd.DataFrame, output: Path) -> None:
    ...
    width = 0.26
    ax.bar([v - width for v in x], annual["strategy_return"], width, color=COLORS["strategy"], label="Momentum Select")
    ...
    ax.set_title("Momentum Select — annual returns by experimental role")
...
def write_year_readme(year_dir: Path, annual: pd.Series, weekly: pd.DataFrame) -> None:
    ...
    lines = [
        ...
        f"- Momentum Select return: **{percent(float(annual['strategy_return']))}**",
